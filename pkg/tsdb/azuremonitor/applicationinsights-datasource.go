...
func (e *ApplicationInsightsDatasource) buildQueries(queries []*tsdb.Query, timeRange *tsdb.TimeRange) ([]*ApplicationInsightsQuery, error) {
	...
	for _, query := range queries {
		...
		if rawQuery {
			rawQuery, err := KqlInterpolate(query, timeRange, fmt.Sprintf("%v", rawQuery))
			if err != nil {
				return nil, err
			}
...
func (e *ApplicationInsightsDatasource) executeQuery(ctx context.Context, query *ApplicationInsightsQuery) (*tsdb.QueryResult, error) {
	...
	if query.IsRaw {
		queryResult.Series, err = e.parseTimeSeriesFromQuery(body, query)
		if err != nil {
			queryResult.Error = err
			return queryResult, nil
		}
...
func (e *ApplicationInsightsDatasource) parseTimeSeriesFromQuery(body []byte, query *ApplicationInsightsQuery) (tsdb.TimeSeriesSlice, error) {
	...
	for _, t := range data.Tables {
		if t.Name == "PrimaryResult" {
			...
			if timeIndex == -1 {
				return nil, errors.New(fmt.Sprintf("could not find time column '%s', found %v", query.TimeColumnName, names))
			}

			if valueIndex == -1 {
				return nil, errors.New(fmt.Sprintf("could not find value column '%s', found %v", query.ValueColumnName, names))
			}
